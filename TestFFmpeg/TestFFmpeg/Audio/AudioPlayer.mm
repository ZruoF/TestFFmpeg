...
// According to Apple guide, the number of audio queue buffers is recommended to
// 3
#define NUM_BUFFER 3
...
@interface AudioPlayer () {
  ...
  AudioQueueBufferRef mBuffers[NUM_BUFFER]; // An array holding pointers to the
                                            // audio queue buffer
  AudioFileID
      mAudioFile; // Audio file representing the audio file you want to play
  UInt32 bufferByteSize; // size in bytes for each audio queue, use
                         // DervieBufferSize to get
  SInt64 mCurrentPacket; // The packet index for the next packet to play in your
                         // audiio
  UInt32 mNumPacketsToRead; // Number of packets to read on each audio queue's
                            // callback
  AudioStreamPacketDescription *mPacketDescs; // For VBR audio data, the array
                                              // of packet descriptions for the
                                              // file being played. For CBR
                                              // data, the value of this field
                                              // is NULL.
  bool
      mIsRunning; // A boolean value indicates the audio queue is running or not
  // =============================================================
...
- (int)DeriveBufferSize:(AudioStreamBasicDescription)ASBdescription
         withPakcetSize:(UInt32)maxPacketSize
             andSeconds:(Float64)seconds;
@end

@implementation AudioPlayer

// Reference to Apple "Audio Queue Service Programming Guide" as
// HandleOutputBuffer Function
void HandleOutputBuffer(void *aqData, AudioQueueRef inAQ,
                        AudioQueueBufferRef inBuffer) {
  // inAQ owns the audio queue's callback
  // inBuffer is an audio queue buffer that the callback is to fill with data by
  // reading from an audio file
  AudioPlayer *player = (__bridge AudioPlayer *)aqData;
  [player putAVPacketsIntoAudioQueue:inBuffer];
}

- (id)initAuido:(AudioPacketQueue *)audioQueue
  ...
  if (audio_index >= 0) {
    ...
    default:
      NSLog(@"Error: audio format '%s' (%d) is not supported",
            pAudioCodecCtx->codec_name, pAudioCodecCtx->codec_id);
    ...
    if (audioFormat.mFormatID != -1) {
      ...
      audioFormat.mBitsPerChannel =
          8 * av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
      audioFormat.mChannelsPerFrame = pAudioCodecCtx->channels;
      audioFormat.mBytesPerFrame =
          pAudioCodecCtx->channels * av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
      ...
      if (pAudioCodecCtx->sample_fmt == AV_SAMPLE_FMT_FLTP) {
        if (pAudioCodecCtx->channel_layout != 0) {
          pSwrCtx = swr_alloc_set_opts(
              pSwrCtx, pAudioCodecCtx->channel_layout, AV_SAMPLE_FMT_S16,
              pAudioCodecCtx->sample_rate, pAudioCodecCtx->channel_layout,
              AV_SAMPLE_FMT_FLTP, pAudioCodecCtx->sample_rate, 0, 0);
        } else {
          pSwrCtx = swr_alloc_set_opts(
              pSwrCtx, pAudioCodecCtx->channels + 1, AV_SAMPLE_FMT_S16,
              pAudioCodecCtx->sample_rate, pAudioCodecCtx->channels + 1,
              AV_SAMPLE_FMT_FLTP, pAudioCodecCtx->sample_rate, 0, 0);
        }
        NSLog(@"sample_rate=%d, channels=%d, channel_layout=%lld",
        ...
        pSwrCtx = swr_alloc_set_opts(
            pSwrCtx, pAudioCodecCtx->channel_layout, AV_SAMPLE_FMT_S16,
            pAudioCodecCtx->sample_rate, pAudioCodecCtx->channel_layout,
            AV_SAMPLE_FMT_S16P, pAudioCodecCtx->sample_rate, 0, 0);
        if (swr_init(pSwrCtx) < 0) {
        ...
        pSwrCtx = swr_alloc_set_opts(
            pSwrCtx, 1, AV_SAMPLE_FMT_S16, pAudioCodecCtx->sample_rate, 1,
            AV_SAMPLE_FMT_U8, pAudioCodecCtx->sample_rate, 0, 0);
        if (swr_init(pSwrCtx) < 0) {
          NSLog(@"swr_init()  fail");
          return nil;
        }
      ...
      if ((err = AudioQueueNewOutput(&audioFormat, HandleOutputBuffer,
                                     (__bridge void *)(self), NULL, NULL, 0, &mQueue)) !=
          noErr) {
        ...
        vBufferSize = [self DeriveBufferSize:audioFormat
                              withPakcetSize:pAudioCodecCtx->bit_rate / 8
                                  andSeconds:AUDIO_BUFFER_SECONDS];
        for (i = 0; i < AUDIO_BUFFER_QUANTITY; i++) {
          if ((err = AudioQueueAllocateBufferWithPacketDescriptions(
                   mQueue, vBufferSize, 1, &mBuffers[i])) != noErr) {
...
- (void)stop:(BOOL)bStopImmediatelly {
  ...
  // Disposing of the audio queue also disposes of all its resources, including
  // its buffers.
...
- (int)DeriveBufferSize:(AudioStreamBasicDescription)ASBdescription
  ...
  // the upper bound is set to 320
  // KB. This corresponds to approximately five seconds of stereo, 24 bit audio
  ...
  if (ASBdescription.mFramesPerPacket != 0) {
    Float64 numPacketForTime =
        ASBdescription.mSampleRate / ASBdescription.mFramesPerPacket;
...
- (UInt32)putAVPacketsIntoAudioQueue:(AudioQueueBufferRef)audioQueueBuffer {
  ...
  if ([audioPacketQueue count] == 0) {
    ...
    @synchronized(self) {
      ...
      bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
          .mStartOffset = bufferRef->mAudioDataByteSize;
      bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
          .mDataByteSize = vSlienceDataSize;
      bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
  ...
  if (bufferRef->mPacketDescriptionCount <
      bufferRef->mPacketDescriptionCapacity) {

    [audioPacketQueue getAVPacket:&aAudioPacket];

#if DECODE_AUDIO_BY_FFMPEG == 1 // Decode by FFmpeg
    if (bufferRef->mAudioDataBytesCapacity - bufferRef->mAudioDataByteSize >=
        aAudioPacket.size) {
      ...
      while (pktSize > 0) {
        ...
        @synchronized(self) {
          len = avcodec_decode_audio4(pAudioCodecCtx, pAVFrame1, &gotFrame,
                                      &aAudioPacket);
        }
        ...
        if (gotFrame > 0) {
          ...
          int data_size = av_samples_get_buffer_size(
              pAVFrame1->linesize, pAudioCodecCtx->channels,
              pAVFrame1->nb_samples, AV_SAMPLE_FMT_S16, 0);

          if (bufferRef->mAudioDataBytesCapacity -
            ...
            @synchronized(self) {
              ...
              // Convert audio.
              outCount = swr_convert(pSwrCtx, (uint8_t **)(&pOut), in_samples,
              ...
              memcpy((uint8_t *)bufferRef->mAudioData +
                         bufferRef->mAudioDataByteSize,
                     pOut, data_size);
              bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
                  .mStartOffset = bufferRef->mAudioDataByteSize;
              bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
                  .mDataByteSize = data_size;
              bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
                  .mVariableFramesInPacket = 1;
    ...
    if (bufferRef->mAudioDataBytesCapacity - bufferRef->mAudioDataByteSize >=
      ...
      _bIsADTSAAS = [AudioUtilities
          parseAACADTSHeader:pHeader
                    toHeader:(tAACADTSHeaderInfo *)&vxADTSHeader];
      ...
      memcpy((uint8_t *)bufferRef->mAudioData + bufferRef->mAudioDataByteSize,
             aAudioPacket.data + vOffsetOfADTS,
             aAudioPacket.size - vOffsetOfADTS);
      bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
          .mStartOffset = bufferRef->mAudioDataByteSize;
      bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
          .mDataByteSize = aAudioPacket.size - vOffsetOfADTS;
      bufferRef->mPacketDescriptions[bufferRef->mPacketDescriptionCount]
          .mVariableFramesInPacket = aCodecCtx->frame_size;
  ...
  if (bufferRef->mPacketDescriptionCount > 0) {
    ...
    if ((err = AudioQueueEnqueueBuffer(mQueue, bufferRef, 0, NULL)))
#else // VBR
    if ((err = AudioQueueEnqueueBufferWithParameters(mQueue, bufferRef, 0, NULL,
                                                     0, 0, 0, NULL,
...
- (void)decodeAudioFile:(NSString *)filePathIn
  ...
  [AudioUtilities writeWAVHeaderWithCodecCtx:pAudioCodecCtx
                               withFormatCtx:pFormatCtx
                                      toFile:wavFile];
  while (av_read_frame(pFormatCtx, &AudioPacket) > 0) {
    if (AudioPacket.stream_index == audioStream) {
      ...
      while (pktSize > 0) {
        ...
        if (gotFrame > 0) {
          int data_size = av_samples_get_buffer_size(
              NULL, pAudioCodecCtx->channels, pAVFrame1->nb_samples,
