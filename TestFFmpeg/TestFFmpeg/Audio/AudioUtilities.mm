...
+ (BOOL)parseAACADTSHeader:(uint8_t *)input
                  toHeader:(tAACADTSHeaderInfo *)ADTSHeader {
  ...
  ADTSHeader->frame_length =
      ((input[3] & 0x03) << 11) + ((input[4]) << 3) + ((input[5] & 0xE0) >> 5);
  ADTSHeader->adts_buffer_fullness =
      ((input[5] & 0x1F) << 6) + ((input[6] & 0xFC) >> 2);
...
- (uint8_t *)generateAACADTSHeader:(uint8_t *)pInOut
                          ToHeader:(tAACADTSHeaderInfo *)pADTSHeader {
  if (pADTSHeader == nil)
    return NULL;
...
+ (int)getMPEG4AudioSampleRates:(uint8_t)vSamplingIndex {
  int pRates[13] = {96000, 88200, 64000, 48000, 44100, 32000, 24000,
...
+ (void)printFileStreamBasicDescription:
...
+ (void)printFileStreamBasicDescriptionFromFile:(NSString *)filePath {
  ...
  if (size != noErr) {
    NSLog(@"*** Error *** PlayAudio - play:Path: could not open audio file. "
          @"Path given was: %@",
          filePath);
    return;
  } else {
  ...
  AudioFileGetProperty(audioFile, kAudioFilePropertyDataFormat, &size,
                       &dataFormat);
...
+ (void)writeWAVHeaderWithCodecCtx:(AVCodecContext *)pAudioCodecCtx
                     withFormatCtx:(AVFormatContext *)pFormatCtx
                            toFile:(FILE *)wavFile {
  char *data;
  int32_t long_temp;
  int16_t short_temp;
  ...
  audioDataSize = (pFormatCtx->duration) * (vBitsPerSample / 8) *
  ...
  // ByteRate (U32)
  long_temp = (vBitsPerSample / 8) * (pAudioCodecCtx->channels) *
...
+ (id)initForDecodeAudioFile:(NSString *)filePathIn
                   toPCMFile:(NSString *)filePathOutput {
  ...
  avcodec_register_all(); /*Register all the codecs, parsers and bitstream
                             filters which were enabled at configuration time.*/
  ...
  if (avformat_open_input(
          &pAudioFormatCtx,
          [filePathIn cStringUsingEncoding:NSASCIIStringEncoding], NULL,
          NULL) != 0) {
  ...
  if (pAudioCodeCtx->sample_fmt == AV_SAMPLE_FMT_FLTP) {
    pSwrCtx = swr_alloc_set_opts(
        pSwrCtx, pAudioCodeCtx->channel_layout, AV_SAMPLE_FMT_S16,
        pAudioCodeCtx->sample_fmt, pAudioCodeCtx->channel_layout,
        AV_SAMPLE_FMT_FLTP, pAudioCodeCtx->sample_rate, 0, 0);
    if (swr_init(pSwrCtx) < 0) {
      return nil;
    }
  } else if (pAudioCodeCtx->bits_per_raw_sample ==
             8) { // For topview ipcamera pcm_law
    pSwrCtx = swr_alloc_set_opts(
        pSwrCtx, 1, AV_SAMPLE_FMT_S16, pAudioCodeCtx->sample_rate, 1,
        AV_SAMPLE_FMT_U8, pAudioCodeCtx->sample_rate, 0, 0);
    if (swr_init(pSwrCtx) < 0) {
      return nil;
    }
  }
  ...
  [AudioUtilities writeWAVHeaderWithCodecCtx:pAudioCodeCtx
                               withFormatCtx:pAudioFormatCtx
                                      toFile:wavFile];
  ...
  while (av_read_frame(pAudioFormatCtx, &audioPacket) > 0) {
    ...
    if (audioPacket.stream_index == audioStream) {
      ...
      while (pktSize > 0) {
        len = avcodec_decode_audio4(pAudioCodeCtx, pAVFrame, &gotFrame,
                                    &audioPacket);
        if (len < 0) {
        ...
        if (gotFrame) {
          int data_size = av_samples_get_buffer_size(
              NULL, pAudioCodeCtx->channels, pAVFrame->nb_samples,
          ...
          if (pAudioCodeCtx->sample_fmt == AV_SAMPLE_FMT_FLTP) {
            ...
            av_samples_alloc(&output, &out_lineSize, pAVFrame->channels,
                             in_samples, AV_SAMPLE_FMT_S16, 0);
            outCount = swr_convert(pSwrCtx, (uint8_t **)&output, in_samples,
