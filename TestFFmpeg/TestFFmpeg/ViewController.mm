...
//  TestFFmpeg
//
//  Created by zengruofan on 16/8/22.
...
NSString *const kAudioTestPath = @"http://hls.hz.qingting.fm/live/386.m3u8";

typedef enum { kTCP = 0, kUDP } kNetworkWay;
  ...
  if ([btn.currentTitle isEqualToString:@"Stop"]) {
    ...
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          if ([self initFFmpegAudioStream:kAudioTestPath
                          withTransferWay:kTCP] == NO) {
          ...
          aPlayer =
              [[AudioPlayer alloc] initAuido:nil
                                withCodecCtx:(AVCodecContext *)pAudioCodeCtx];
...
- (BOOL)initFFmpegAudioStream:(NSString *)filePath
              withTransferWay:(kNetworkWay)network {
  ...
  [AudioUtilities parseAACADTSHeader:pInput
  ...
  if (strncmp([filePath UTF8String], "rtsp", 4) == 0) {
    ...
    pAudioInPath = filePath;
    pAudioInPath = [pAudioInPath stringByReplacingOccurrencesOfString:@"mms:"
                                                           withString:@"mmsh:"];
    ...
    pAudioInPath =
        [[[NSBundle mainBundle] bundlePath] stringByAppendingString:filePath];
    isLocalFile = YES;
  }
  ...
  case kTCP: {
    ...
    // Open video file
    int errCode;
    if ((errCode = avformat_open_input(
             &pFormatCtx,
             [pAudioInPath cStringUsingEncoding:NSASCIIStringEncoding], NULL,
             &option)) != 0) {
      char buf[1024];
      av_strerror(errCode, buf, 1024);
      printf("Couldn＇t open file : %d(%s)", errCode, buf);
      NSLog(@"Could not open connection %d", errCode);
      return NO;
    }
    av_dict_free(&option);
  } break;
  ...
  if ((audioStream = av_find_best_stream(pFormatCtx, AVMEDIA_TYPE_AUDIO, -1, -1,
                                         &pAudioCodec, 0)) < 0) {
    ...
    if (pAudioCodec->profiles) {
      NSLog(@"Profile names = %@",
            [[NSString alloc] initWithCString:pAudioCodec->profiles->name]);
    } else {
